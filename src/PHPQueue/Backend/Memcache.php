...
class Memcache
    extends Base
    implements KeyValueStore, IndexedFifoQueueStore
{
    public $servers;
    ...
    public function clear($key)
    {
        ...
        if ($this->queue_name) {
            $contents = $this->get_bucket_contents();
            $index = array_search($key, $contents);
    ...
    public function pop() {
        if (!$this->queue_name) {
            throw new BackendException("Can not pop without a queue name");
        }
		...
		//TODO: need cas tokens - use memcached?
        $this->set_bucket_contents($contents);
    ...
    public function push($value) {
        $key = uniqid("PHPQueue-autogenerated:");
        $this->set($key, $value);
        return $key;
    }
